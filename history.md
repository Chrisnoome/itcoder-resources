...
- 09-25 CAPS-only unnumbered lessons: SQLite in Delphi, SQLite in Lazarus, the Grade 10, 11 and 12 PATs (2026 themes as examples), the alternative task; Grade 12 PAT pre-check. Stream visibility (`'stream' => 'caps'`).
- 09-25 Practice: flash cards, hangman, word search and crossword from the glossary (20 words a round), XP, ranks Bit-Petabyte, badges, streaks, daily challenge, leaderboards with a chosen name and emoji icon (bad words filtered, no uploads). 42 programming terms added to the glossary (Operator, Expression, Statement, Iteration, Database ... - 318 terms).
- 09-25 Practice round two: speed match, spaced repetition (practiceWords), a weekly Classes board, and a Practice section on the teacher page (per-pupil practice, most-missed words).
- 09-25 Fun practice redesign: themed game arenas, sounds and effects, a game-style lobby, grade choice (words up to your grade), a crossword that takes typing, and flash cards that listen (whisper.cpp base.en on the VPS, honour system as the fallback).
...
- 09-25 Design preview E (?design=e on a lesson; lib/design.php, design-e.css): outline left, margin notes and blue-pen doodles right; rule: margin content never pushes the text apart. Doodle()/DesignFigure() in lesson 8 and AI lesson 5, preview only.
- 09-25 Pascal: "widening"/"narrowing" no longer banned - lesson 7 to teach them; syllabus gap additions placed in lessons (courses/pascal-course.md, Gap additions).
- 09-25 The E look for every lesson and the pupil pages (home, subjects, courses, lesson lists, marks, account); style B's bottom bar with an answer grid and the day's AI count; about 180 margin doodles and notes across both courses; all 138 Pascal code hints rewritten as short ways of thinking, no code; Practice lobby now refreshes after a round (plan upgrades show).
- 09-25 Pascal gap additions built (lessons 1, 4, 7, 8, 12, 13, 14, 15, 16, 19, 23); try-it `array2d`; lesson 23 screenshots from the new moreforms Lazarus project.
- 09-25 One design for every course: README (Adding a course), platform.md decision 27 and content-voice §5b say design changes apply to Java and every later course too; South African English rule in writing-style.md.
