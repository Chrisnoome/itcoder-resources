...
procedure CaptureHandle(aHandle: HWND; const aName: string);
var OutDir: string;
implementation
...
procedure CaptureHandle(aHandle: HWND; const aName: string);
var cdc, mdc: HDC; hb, hb2, old, old2: HBITMAP; wr, fr: TRect; crop: TBitmap; png: TPortableNetworkGraphic;
begin
  Settle(600);
  GetWindowRect(aHandle, wr);
  if DwmGetWindowAttribute(aHandle, 9, @fr, SizeOf(fr)) <> 0 then fr := wr;
  crop := TBitmap.Create; png := TPortableNetworkGraphic.Create;
  try
    mdc := CreateCompatibleDC(0);
    hb := CreateCompatibleBitmap(GetDC(0), wr.Right - wr.Left, wr.Bottom - wr.Top);
    old := SelectObject(mdc, hb);
    PrintWindow(aHandle, mdc, 2);
